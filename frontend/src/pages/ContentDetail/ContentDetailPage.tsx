  ...
  return (
    <div className="space-y-10">
      <section className="relative -mx-4 overflow-hidden sm:-mx-6">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-8 px-4 py-10 sm:px-6 md:grid-cols-[260px,1fr] md:items-center">
          {posterUrl ? (
            <img
              src={posterUrl}
              alt={data.title}
              className="aspect-[2/3] w-full max-w-[260px] rounded-xl shadow-card ring-1 ring-white/10"
            />
          ) : (
          ...
          <div className="text-balance">
            <p className="mb-2 text-xs font-bold uppercase tracking-[0.2em] text-accent">
              {data.type === 'movie' ? t('content.movie') : t('content.tv')}
              {dateDisplay}
            </p>
            ...
            <div className="mt-4 flex flex-wrap items-center gap-x-5 gap-y-2 text-sm text-ink-muted">
              ...
              <span>
                {data.voteCount.toLocaleString()} {t('content.tmdbVotes')}
              </span>
              {data.communityRating !== null && (
                <span className="text-accent">
                  {t('content.communityRating')} <strong>{data.communityRating.toFixed(1)}</strong>{' '}
                  · {data.communityReviewCount} {t('content.reviews')}
                </span>
              )}
            </div>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {data.genres.map((g) => (
                <span
                  key={g.id}
                  className="rounded-full bg-surface-raised px-2.5 py-0.5 text-xs text-ink-muted ring-1 ring-white/5"
                >
                  {g.name}
                </span>
              ))}
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <button type="button" className="btn-outline">
                {t('content.addWatchlist')}
              </button>
              {trailer && (
                <button type="button" className="btn-ghost" onClick={() => setTrailerOpen(true)}>
                  ▶ {t('content.trailer')}
                </button>
              )}
            </div>
        ...
        <section>
          <div className="mb-4 flex items-center justify-between">
            <div className="section-title !mb-0">
              <h2>{t('content.cast')}</h2>
            </div>
            <div className="flex gap-2">
              <button
                type="button"
                onClick={() => scrollCast('left')}
                className="grid h-8 w-8 place-items-center rounded-full bg-surface-raised text-ink transition-colors hover:bg-surface-muted ring-1 ring-white/5"
          ...
          <div 
            ref={castScrollRef}
            className="flex snap-x snap-mandatory gap-4 overflow-x-auto pb-4 scrollbar-none [&::-webkit-scrollbar]:hidden"
          >
            {data.cast.map((member) => (
              <article key={member.id} className="card-hover w-36 shrink-0 snap-start overflow-hidden p-0 ring-white/5 sm:w-40">
                {member.profilePath ? (
                  <img
                    src={profile(member.profilePath, 'w185')!}
                    alt={member.name}
                    className="aspect-[2/3] w-full object-cover"
                    loading="lazy"
                  />
                ) : (
                  <div className="grid aspect-[2/3] place-items-center bg-gradient-to-br from-surface-muted to-surface-raised text-2xl font-bold text-ink-muted">
        ...
        >
          <div className="relative w-full max-w-4xl" onClick={(e) => e.stopPropagation()}>
            <button
              type="button"
              onClick={() => setTrailerOpen(false)}
              className="absolute -top-10 right-0 text-sm text-ink-muted hover:text-ink"
            >
              ✕ {t('content.close')}
            </button>
            <div className="aspect-video overflow-hidden rounded-xl shadow-card ring-1 ring-white/10">
              <iframe
                src={youtubeEmbed(trailer.key)}
                title={trailer.name}
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            {data.videos.map((v) => (
              <button
                ...
                className="card-hover relative overflow-hidden p-0 text-left"
              >
                <img
